...
_reload_haproxy_from_status_unlocked() {
    local STATUS_LIST HEALTHY PID_FILE NEW_CFG HAP_BIND_ADDR HAP_BIND_PORT
    mkdir -p "$INSTANCE_STATE_DIR"
    PID_FILE=$(get_haproxy_pid_file)
    ...
    render_haproxy_config "$HAP_BIND_ADDR" "$HAP_BIND_PORT" "$STATUS_LIST" > "$NEW_CFG"
    refresh_healthy_snapshot
    ...
    if [ -f "$HAPROXY_CFG" ] && cmp -s "$NEW_CFG" "$HAPROXY_CFG" 2>/dev/null; then
        rm -f "$NEW_CFG"
        if refresh_haproxy_pid; then
            echo "==> [MicroWARP] HAProxy 配置未变（bind ${HAP_BIND_ADDR}:${HAP_BIND_PORT}，健康 ${HEALTHY}/${WARP_INSTANCE_COUNT}），跳过 reload"
            return 0
        fi
        echo "==> [MicroWARP] HAProxy 配置未变但进程不在，冷启动..."
        if haproxy_cold_start "$HAPROXY_CFG"; then
            echo "==> [MicroWARP] 🚀 HAProxy 已上线 (监听: ${HAP_BIND_ADDR}:${HAP_BIND_PORT}, master PID: ${HAPROXY_PID})"
            for _iid in $(get_instance_ids "$WARP_INSTANCE_COUNT"); do
    ...
    echo "==> [MicroWARP] 刷新 HAProxy 静态配置（bind ${HAP_BIND_ADDR}:${HAP_BIND_PORT}；健康标记 ${HEALTHY}/${WARP_INSTANCE_COUNT}）"
