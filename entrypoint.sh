...
# hev UDP bind port. Assigned/draining inst uses that service's TCP port so
# ASSOCIATE BND.PORT matches the client the client already uses.
get_instance_socks_udp_port() {
    local INST_ID="$1"
    local SID PORT
    SID=$(get_instance_assigned_service "$INST_ID" 2>/dev/null || printf '')
    if [ -z "$SID" ]; then
        SID=$(get_instance_drain_service "$INST_ID" 2>/dev/null || printf '')
    fi
    if [ -n "$SID" ]; then
        PORT=$(get_proxy_port "$SID")
        if [ -n "$PORT" ]; then
            printf '%s\n' "$PORT"
            return 0
        fi
...
# Public DNAT dport for this inst. Empty = no public UDP (unassigned standby,
# or drain whose service port already belongs to the new owner).
get_instance_public_udp_port() {
    local INST_ID="$1"
    local SID PORT OWNER
    SID=$(get_instance_assigned_service "$INST_ID" 2>/dev/null || printf '')
    if [ -n "$SID" ]; then
        get_proxy_port "$SID"
        return 0
    ...
    OWNER=$(get_service_assigned_instance "$SID" 2>/dev/null || printf '')
    if [ -n "$OWNER" ] && [ "$OWNER" != "$INST_ID" ]; then
        printf ''
        return 0
    fi
    get_proxy_port "$SID"
