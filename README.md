...
>
> 多实例运行细节：启动时 **HAProxy 会立刻监听**（即使暂时 0 后端）；**每个实例一旦启动完成就立刻测活并加入池**，不必等全部 N 条隧道都起来才开放服务。实例仍**串行启动并间隔 1 秒错峰**；健康巡检按 `TEST_URLS_CHECK_INTERVAL / WARP_INSTANCES` 错峰。失败实例会**先踢出 HAProxy**（soft-reload），内部 SOCKS 先保留；等 busy 客户端连接排空（或超过 `INSTANCE_DRAIN_TIMEOUT`，默认 **30 秒**）后再停 SOCKS，并由各自的**后台 worker**独立复活（重连 → 重注册，带退避）。若某个实例**获取配置连续失败 3 次**，不会再拖垮整容器退出，而是进入**后台串行配置重试队列**（FIFO + 退避）继续注册。各 worker/队列对注册 API 的请求会串行化，降低打爆注册接口的风险。
>
